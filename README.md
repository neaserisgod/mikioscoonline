...
- **Impresión en posnet** (`src/lib/providers/impresion/mercadopago-terminal.ts`, detrás de `getImpresionProvider()`, consumido por `impresionService`): si `Organization.imprimirTicketPosnet` está activo, cada venta manda un tiquet con ítems/total/recargo y, si ya hay CAE, los datos fiscales + QR, a la terminal Point marcada como posnet — vía `POST /terminals/v1/actions` de MercadoPago (independiente de cualquier cobro). El contenido del tiquet (fiscal o no fiscal) se arma una sola vez en `src/domain/ticket.ts` — ver el comentario en `mercadopago-terminal.ts` sobre el comportamiento no documentado del tag `{left}` (alinea a la derecha, no a la izquierda) verificado a mano contra una terminal Newland N950 real.
