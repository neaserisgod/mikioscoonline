...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (nombre !== undefined) await ubicacionService.editar(id, result.user.organizationId, { nombre })
    if (activo === false) await ubicacionService.desactivar(id, result.user.organizationId)
    if (activo === true) await ubicacionService.reactivar(id, result.user.organizationId)
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params
  try {
    await ubicacionService.eliminar(id, result.user.organizationId)
    return NextResponse.json({ ok: true })
  } catch (e) {
