...
export function CarritoItemsList({ checkout, className }: CarritoItemsListProps) {
  ...
  return (
    <div className={cn("overflow-y-auto rounded-2xl bg-card shadow-[var(--shadow-card)] ring-1 ring-foreground/[0.04] divide-y divide-border/40", className)}>
      <AnimatePresence initial={false}>
        {carrito.map((item) => (
          <motion.div
            ...
            className="px-4 py-3.5 transition-colors hover:bg-muted/25"
          >
            <div className="flex items-start gap-3">
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-medium truncate">{item.nombre}</p>
            ...
            {item.esPesable ? (() => {
              ...
              return (
                <div className="flex items-center gap-1.5 mt-2">
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); (document.querySelector("[data-pos-search-input]") as HTMLInputElement | null)?.focus() } }}
                    onChange={(e) => setGramos(item.productId, Number(e.target.value) || 0)}
                    className={cn(
                      "h-8 w-24 rounded-lg border bg-background px-2 text-sm font-semibold tabular-nums",
              ...
              return (
                <div className="flex items-center gap-1.5 mt-2">
                  <Button variant="outline" size="icon-sm" className="size-8 rounded-lg border-border/60"
                  ...
                  <input
                    ...
                    className={cn(
                      "h-8 w-14 rounded-lg border bg-background px-1.5 text-center text-sm font-semibold tabular-nums",
