...
interface AbrirCajaSheetProps {
  ...
  /** false = caja digital (MP/QR/posnet). Abre siempre en $0 a la vista, para
   * cualquier rol — no se pide ni muestra fondo inicial acá; el backend
   * arrastra el saldo real del último cierre por detrás (ver cajaSesion.service.ts). */
  manejaEfectivo: boolean
  onSuccess: () => void
}

/** Contenido de apertura de caja. El caller lo envuelve en su propio <Sheet><SheetContent>. */
export function AbrirCajaSheet({ cajaNombre, cajaId, fondoSugerido, manejaEfectivo, onSuccess }: AbrirCajaSheetProps) {
  ...
  async function onSubmit(data: z.infer<typeof abrirFormSchema>) {
    try {
      const pesos = manejaEfectivo ? data.pesos : 0
      await abrirCajaAction(cajaId, { fondoInicialCentavos: Math.round(pesos * 100) })
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-6 space-y-4">
        {manejaEfectivo ? (
          ...
          <p className="text-sm text-muted-foreground">
            Se abre en $0 a la vista — el sistema sigue el saldo real por detrás, no se pierde lo acumulado.
          </p>
        )}
